...
type Props = {
  featuredRoom?: Room | null; // make it optional
};

const FeaturedRoom: FC<Props> = ({ featuredRoom }) => {
  if (!featuredRoom) {
    return <p className="text-center py-10">No featured room available</p>;
  }

  const coverImageUrl = featuredRoom.coverImage?.url || '/placeholder.jpg';
  const roomImages = featuredRoom.images?.slice(1, 3) || [];
    ...
    <section className="flex md:flex-row flex-col px-4 py-10 items-center gap-12 container mx-auto">
      ...
      <div className="md:py-10 md:w-1/2 text-left">
        ...
        <p className="font-normal max-w-md">
          {featuredRoom.description || 'No description available.'}
        </p>
